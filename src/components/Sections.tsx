...
export function About(){
  const {copy}=useLanguage()
  return <section id="about" className="bg-white"><div className="section-pad grid gap-14 lg:grid-cols-2 lg:items-start"><div><Heading eye={copy.about.eye} title={copy.about.title}/><div className="mt-10 space-y-5">{copy.about.p.map(text=><motion.p {...fade} key={text} className="body-lg">{text}</motion.p>)}</div></div><motion.div {...fade} className="relative lg:sticky lg:top-28"><img src="/images/about-avala.jpg" alt="Professional K9 handler working with a dog" className="aspect-[4/5] w-full object-cover object-[center_22%]" loading="lazy"/></motion.div></div></section>
}

export function Services(){
  const {copy}=useLanguage()
  return <section id="services" className="section-pad bg-paper"><Heading eye={copy.services.eye} title={copy.services.title}/><motion.div {...fade} className="mt-8 max-w-3xl space-y-5"><p className="body-lg">{copy.services.p}</p><p className="body-lg">{copy.services.quality}</p></motion.div><div className="mt-14 grid gap-5 lg:grid-cols-2">{copy.services.cards.map(([num,title,desc,cta],index)=><a key={title} href={index?'#dual-purpose':'#detection'} className="media-card group relative min-h-[480px] overflow-hidden bg-ink p-7 text-white md:p-10"><img src={index?'/images/gallery/k9-03.jpg':'/images/gsp-working-01.jpg'} alt="Professional working K9 training" className="reveal-img absolute inset-0 h-full w-full object-cover opacity-60 transition duration-700 group-hover:scale-[1.03]" loading="lazy"/><div className="absolute inset-0 bg-gradient-to-t from-black via-black/25 to-transparent"/><div className="relative flex h-full flex-col justify-between"><span className="text-xs font-bold tracking-[.2em] text-sand">{num}</span><div><h3 className="text-4xl font-semibold tracking-tight">{title}</h3><p className="mt-4 max-w-md text-sm leading-7 text-white/75">{desc}</p><span className="mt-6 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.13em] text-sand">{cta}<ArrowUpRight size={16}/></span></div></div></a>)}</div></section>
}
...
export function Gsp(){
  const {copy}=useLanguage()
  return <section className="grid bg-paper lg:grid-cols-[1.1fr_.9fr]"><div className="relative min-h-[520px] lg:min-h-[850px]"><img src="/images/gsp-working-02.jpg" alt="German Shorthaired Pointer performing detection work" className="absolute inset-0 h-full w-full object-cover" loading="lazy"/><div className="absolute bottom-6 right-0 bg-sand p-7 text-ink md:p-10"><strong className="block text-6xl tracking-[-.06em] md:text-8xl">95%+</strong><span className="mt-2 block max-w-44 text-xs font-bold uppercase tracking-[.14em]">{copy.gsp.stat}</span></div></div><div className="section-pad !mx-0 !max-w-none flex flex-col justify-center"><Heading eye={copy.gsp.eye} title={copy.gsp.title}/><div className="mt-10 space-y-5">{copy.gsp.p.map(text=><p className="body-lg" key={text}>{text}</p>)}</div></div></section>
}

export function Dual(){
  const {copy}=useLanguage()
  return <section id="dual-purpose" className="relative overflow-hidden bg-ink text-white"><div className="absolute inset-y-0 right-0 hidden w-[42%] lg:block"><img src="/images/gallery/k9-04.jpg" alt="Controlled professional dual-purpose K9 training" className="h-full w-full object-cover opacity-35" loading="lazy"/><div className="absolute inset-0 bg-gradient-to-r from-ink to-transparent"/></div><div className="section-pad relative z-10"><div className="max-w-3xl"><Heading eye={copy.dual.eye} title={copy.dual.title} light/>{copy.dual.p.map(text=><p className="mt-6 max-w-2xl text-base leading-8 text-white/65" key={text}>{text}</p>)}</div><h3 className="mt-16 text-xl font-semibold">{copy.dual.sub}</h3><ul className="mt-5 grid max-w-5xl gap-x-9 md:grid-cols-2 lg:grid-cols-3">{copy.dual.items.map(item=><Feature key={item}>{item}</Feature>)}</ul><p className="mt-12 max-w-3xl border-l-2 border-sand pl-6 text-lg leading-8 text-white/75">{copy.dual.final}</p></div></section>
}
