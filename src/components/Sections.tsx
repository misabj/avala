...
const serviceImages = [
  ...
  { src: '/images/shepherd-close-up.jpg', alt: 'German Shepherd during controlled protection training' },
]
...
export function Dual() {
  const { copy } = useLanguage()
  return <section id="dual-purpose" className="relative overflow-hidden bg-ink text-white"><div className="relative aspect-video overflow-hidden lg:absolute lg:inset-y-0 lg:right-0 lg:w-[52%] lg:aspect-auto"><img src="/images/shepherd-training.jpg" alt="German Shepherd in controlled protection training with a handler" className="h-full w-full object-cover object-center opacity-90 lg:object-left lg:opacity-50" loading="lazy" /><div className="absolute inset-0 bg-gradient-to-t from-ink/25 to-transparent lg:bg-gradient-to-r lg:from-ink lg:via-ink/45 lg:to-transparent" /></div><div className="section-pad relative z-10"><div className="max-w-3xl"><Heading eye={copy.dual.eye} title={copy.dual.title} light />{copy.dual.p.map(text => <p className="mt-6 max-w-2xl text-base leading-8 text-white/65" key={text}>{text}</p>)}</div><h3 className="mt-16 text-xl font-semibold">{copy.dual.sub}</h3><ul className="mt-5 grid max-w-5xl gap-x-9 md:grid-cols-2 lg:grid-cols-3">{copy.dual.items.map(item => <Feature key={item}>{item}</Feature>)}</ul><p className="mt-12 max-w-3xl border-l-2 border-sand pl-6 text-lg leading-8 text-white/75">{copy.dual.final}</p></div></section>
}
